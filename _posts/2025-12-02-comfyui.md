...
**ComfyUI 是一套高度模組化、可視化的 Stable Diffusion 工作流程工具。**
與一般的圖像生成介面（如 Automatic1111）不同，它讓你能看見整個生成管線的細節，包括：
...
Comfy CLI 是 ComfyUI 官方提供的命令列工具，可以大幅簡化安裝與管理流程。([ComfyUI Wiki][2])
...
* **建立虛擬環境（推薦）**
  ...
  > 提醒：每次使用 ComfyUI 前，都需要 activate 虛擬環境。要退出則用 `deactivate`。([ComfyUI Wiki][1])
...
預設情況下，ComfyUI 將在 `http://localhost:8188` 運行。([ComfyUI Wiki][1])
...
4. **安裝 GPU 支援**
   根據你的 GPU 類型，安裝對應版本的 PyTorch 類似前述方法。([ComfyUI Wiki][1])
...
* 手動從模型來源（例如 Hugging Face）下載模型檔案，然後放到 `models/checkpoints/` 目錄。([ComfyUI Wiki][2])
...
---
[1]: https://comfyui-wiki.com/zh/install/install-comfyui/install-comfyui-on-linux?utm_source=chatgpt.com "Linux 系统安装 ComfyUI 教程 | ComfyUI Wiki"
